...
function App() {
  ...
  return (
    <div className="site-shell">
      ...
      <footer><button className="brand footer-brand" onClick={() => scrollTo("inicio")}><span className="brand-icon"><Flame size={22}/></span><span><strong>LUMÉ</strong><small>FORNERIA</small></span></button><div className="footer-links">{nav.map(([id,label]) => <button key={id} onClick={() => scrollTo(id)}>{label}</button>)}</div><div className="footer-social"><button><Instagram/></button><button><Heart/></button></div><p>Projeto conceitual desenvolvido pela LUDO Digital. Marca, pessoas e informações fictícias.</p></footer>
